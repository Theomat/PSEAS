...
class ASlibScenario(object):
    ...
    def read_from_csv(
        ...
        self.algorithms = list(self.performance_data.columns)  # list of strings
        # self.algortihms_deterministics = self.algorithms  # list of strings
        # self.algorithms_stochastic = []  # list of strings

        self.features_deterministic = list(self.feature_data.columns)  # list of strings
        self.features_stochastic = []  # list of strings
        self.feature_group_dict = {"all": {"provides": self.features_deterministic}}
        ...
        self.runstatus_data = pd.DataFrame(
            data=np.array([["ok"] * len(self.algorithms)] * len(self.instances)),
            index=self.performance_data.index,
            columns=self.performance_data.columns,
        )

        if objective == "runtime":
            self.runstatus_data[self.performance_data >= runtime_cutoff] = "timeout"
    ...
    def read_description(self, fn):
        ...
        self.performance_measure = (
            ...
            if isinstance(description.get("performance_measures"), list)
            else [description.get("performance_measures")]
        )
        ...
        self.performance_type = (
            performance_type
            if isinstance(performance_type, list)
            else [performance_type]
        )

        self.algorithm_cutoff_time = description.get("algorithm_cutoff_time")
        self.algorithm_cutoff_memory = description.get("algorithm_cutoff_memory")
        ...
        if not self.maximize or self.maximize == "?":
            ...
            error_found = True
        if (not self.algorithm_cutoff_time or self.algorithm_cutoff_time == "?") and (
            self.performance_type == "quality"
        ):
    ...
    def read_algorithm_runs(self, fn):
        ...
        if arff_dict["attributes"][0][0].upper() != "INSTANCE_ID":
            self.logger.error("instance_id as first attribute is missing in %s" % (fn))
            sys.exit(3)
        if arff_dict["attributes"][1][0].upper() != "REPETITION":
            self.logger.error("repetition as second attribute is missing in %s" % (fn))
            sys.exit(3)
        if arff_dict["attributes"][2][0].upper() != "ALGORITHM":
            self.logger.error("algorithm as third attribute is missing in %s" % (fn))
            sys.exit(3)
        ...
        if arff_dict["attributes"][3 + i][0].upper() != "RUNSTATUS":
            self.logger.error("runstatus as last attribute is missing in %s" % (fn))
            sys.exit(3)
        ...
        perf_data = pd.DataFrame(
            arff_dict["data"], columns=algo_inst_col + perf_col + status_col
        )

        # group performance data by mean value across repetitions
        for perf in self.performance_measure:
            self.performance_data_all.append(
                perf_data.groupby(["instance_id", "algorithm"])
                .median(numeric_only=True)
                .unstack("algorithm")[perf]
            )
        ...
        self.runstatus_data = (
            perf_data.groupby(["instance_id", "algorithm"])["runstatus"]
            .aggregate(lambda x: collections.Counter(x).most_common(1)[0][0])
            .unstack("algorithm")
        )
    ...
    def read_feature_values(self, fn):
        ...
        feature_set = set(self.features_deterministic).union(self.features_stochastic)
        ...
        for data in arff_dict["data"]:
            ...
            if last_name != inst_name:
                self.index_to_instance.append(inst_name)
                last_name = inst_name
            # ===================================================================
        ...
        self.feature_data = self.feature_data.groupby(["instance_id"]).aggregate("mean")
        ...
        if duplicates > 0:
            ...
            self.logger.warn(
                self.feature_data[self.feature_data.duplicated(keep=False)].index
            )
    ...
    def read_feature_costs(self, fn):
        ...
        # instance panda
        cols = list(map(lambda x: x[0], arff_dict["attributes"][:1]))
        instance_data = pd.DataFrame(data[:, :1], columns=cols)

        self.feature_cost_data = pd.concat(
            [instance_data, imputed_feature_cost_data], axis=1
        )
    ...
    def read_feature_runstatus(self, fn):
        ...
        if len(self.feature_group_dict.keys()) != len(arff_dict["attributes"][2:]):
            self.logger.error(
                ...
                % (
                    len(self.feature_group_dict.keys()),
        ...
        self.feature_runstatus_data = self.feature_runstatus_data.drop(
            "repetition", axis=1
        )
    ...
    def read_cv(self, fn):
        ...
        if arff_dict["attributes"][2][0].upper() != "FOLD":
            self.logger.error("fold as third attribute is missing in %s" % (fn))
            sys.exit(3)
    ...
    def check_data(self, alpha: float = 10):
        ...
        for perf_type_i, perf_type in enumerate(self.performance_type):
            ...
            if perf_type == "runtime":
                ...
                self.performance_data_all[perf_type_i][self.runstatus_data != "ok"] = (
                    self.algorithm_cutoff_time * alpha
                )
        ...
        for data in all_data:
            if data is not None and set_insts.difference(data.index):
                self.logger.error(
                    "Not all data matrices have the same instances: %s"
    ...
    def create_cv_splits(self, n_folds: int = 10):
        ...
        self.cv_data = pd.DataFrame(
            data=np.zeros(len(self.instances)),
        ...
        for indx, (train, test) in enumerate(kf.split(self.instances)):
            ...
            self.cv_data.iloc[test] = indx + 1.0
