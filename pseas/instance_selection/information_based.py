...
def __instance_parameters__(self, state, loc: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    indices_not_run: List[int] = [
    ...
    scale_wo_instance: np.ndarray = np.zeros(
        len(indices_not_run), dtype=np.float64)  # parameter for p(X|Y=y)
    for j, i in enumerate(indices_not_run):
        scale_wo_instance[j] = max(1e-20, 
        self._sum_scales([self._scales[k] for k in indices_not_run if i != k]))
    # Expected error for each instance
    exp_err_for_instance: np.ndarray = np.array([-x for i, x in enumerate(state[1])
    ...
    return expected_error_wo_instance, scale_wo_instance, exp_err_for_instance, inst_scales
...
def __compute_information_relative_to_decision__(self, state, k: float = 5):
    ...
    observed_error, loc, scale = __global_error_parameters__(self, 
        state)
    expected_error_wo_instance, scale_wo_instance, inst_locs, inst_scales = __instance_parameters__(self, 
        state, loc)
    ...
    info_gained: np.ndarray = np.maximum(rel_entr(
        x_is_zero_knowing_y, x_is_zero) + rel_entr(1 - x_is_zero_knowing_y, 1 - x_is_zero), 0)
    # P(Y=y)
    prob = prob_y(y)
    # IG_i = int_y p(Y=y) sum_x p(X=x|Y=y) log (p(X=x|Y=y)/p(X=x))
