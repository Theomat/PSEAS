...
class DistanceBased(InstanceSelection):
    ...
    def ready(self, features: np.ndarray, distributions: np.ndarray, **kwargs) -> None:
        self._weight_matrix = __compute_distance_matrix__(
            features, self._distance)
    ...
    def feed(self, state: Tuple[List[Optional[float]], List[float]]) -> None:
        ...
        new_instances: List[int] = [
            i for i, time in enumerate(state[0]) if time is not None and i not in self._instances_run]
        ...
        mask: np.ndarray = np.array(
                [time is not None for time in state[0]])
